...
const mac_addr_t PEER_BASE_MAC = {0x02, 'n', 'n',
                                  'c',  't', 0x00};  // [5]: peer_id
...
class Frame {
 public:
  ...
  static bool is_pairing_request(const std::vector<uint8_t>& frame,
                                 const mac_addr_t& mac_addr);
  static bool is_pairing_accept(const std::vector<uint8_t>& frame,
                                const mac_addr_t& mac_addr);
  static bool parse_message(const std::vector<uint8_t>& frame,
